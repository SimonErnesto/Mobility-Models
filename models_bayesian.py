...
with pm.Model() as mod_g:
    ...
    gamma = pm.HalfNormal("gamma", g_s, shape=len(Vij_obs))
...
ax[0,1].scatter(Vij_obs_s[:85], Vij_vis_m[:85], marker="o", facecolor="w", color="g", label="Visitation: SSI="+str(round(SSI_v_vis, 2)))
ax[0,1].scatter(Vij_obs_s[:85], Vij_rad_m[:85], marker="^", facecolor="w", color="b", label="Radiation: SSI="+str(round(SSI_v_rad, 2)))
ax[0,1].scatter(Vij_obs_s[:85], Vij_gra_m[:85], marker="x", color="r", label="Gravity: SSI="+str(round(SSI_v_gra, 2)))
ax[0,1].legend()
...
ax[1,1].scatter(Qij_obs_s[:87], Qij_vis_m[:87], marker="o", facecolor="w", color="g", label="Visitation: SSI="+str(round(SSI_q_vis, 2)))
SSI_q_rad2 = 2*np.sum(np.minimum(Qij_obs_s[:87], Qij_rad_m[:87]))/(np.sum(Qij_obs_s[:87]) + np.sum(Qij_rad_m[:87]))
ax[1,1].scatter(Qij_obs_s[:87], Qij_rad_m[:87], marker="^", facecolor="w", color="b", label="Radiation: SSI="+str(round(SSI_q_rad, 2)))
SSI_q_gra2 = 2*np.sum(np.minimum(Qij_obs_s[:87], Qij_gra_m[:87]))/(np.sum(Qij_obs_s[:87]) + np.sum(Qij_gra_m[:87]))
ax[1,1].scatter(Qij_obs_s[:87], Qij_gra_m[:87], marker="x", color="r", label="Gravity: SSI="+str(round(SSI_q_gra, 2)))
...
ax[0,0].plot(np.arange(118), Vij_obs_s, color='k', linestyle=":", label="Observed")
ax[0,0].plot(np.arange(118), Vij_vis.mean_v, color="g", label="Visitation: SSI="+str(round(SSI_v_vis, 2)))
ax[0,0].fill_between(np.arange(118), Vij_vis.h5_v, Vij_vis.h95_v, color="g", alpha=0.2)
ax[0,0].plot(np.arange(118), Vij_gra.mean_v, linestyle="--", color="purple", label="Gravity: SSI="+str(round(SSI_v_gra, 2)))
ax[0,0].fill_between(np.arange(118), Vij_gra.h5_v, Vij_gra.h95_v, color="purple", alpha=0.2)
ax[0,0].legend()
ax[0,0].grid(alpha=0.2)
ax[0,0].set_title("Average Daily Trips")
ax[0,0].set_ylabel("Estmiate")
...
SSI_v_vis2 = 2*np.sum(np.minimum(Vij_obs_s[:85], Vij_vis_m[:85]))/(np.sum(Vij_obs_s[:87]) + np.sum(Vij_vis_m[:85]))
SSI_v_gra2 = 2*np.sum(np.minimum(Vij_obs_s[:85], Vij_gra_m[:85]))/(np.sum(Vij_obs_s[:87]) + np.sum(Vij_gra_m[:85]))
ax[0,1].plot(np.arange(118)[:85], Vij_obs_s[:85], color='k', linestyle=":", label="Observed")
ax[0,1].plot(np.arange(118)[:85], Vij_vis.mean_v[:85], color="g", label="Visitation: SSI="+str(round(SSI_v_vis, 2)))
ax[0,1].fill_between(np.arange(118)[:85], Vij_vis.h5_v[:85], Vij_vis.h95_v[:85], color="g", alpha=0.2)
ax[0,1].plot(np.arange(118)[:85], Vij_gra.mean_v[:85], linestyle="--", color="purple", label="Gravity: SSI="+str(round(SSI_v_gra, 2)))
ax[0,1].fill_between(np.arange(118)[:85], Vij_gra.h5_v[:85], Vij_gra.h95_v[:85], color="purple", alpha=0.2)
ax[0,1].legend()
ax[0,1].grid(alpha=0.2)
...
ax[1,0].plot(np.arange(118), Qij_obs_s, color='k', linestyle=":", label="Observed")
ax[1,0].plot(np.arange(118), Qij_vis.mean_q, color="g", label="Visitation: SSI="+str(round(SSI_v_vis, 2)))
ax[1,0].fill_between(np.arange(118), Qij_vis.h5_q, Qij_vis.h95_q, color="g", alpha=0.2)
ax[1,0].plot(np.arange(118), Qij_gra.mean_q, linestyle="--", color="purple", label="Gravity: SSI="+str(round(SSI_v_gra, 2)))
ax[1,0].fill_between(np.arange(118), Qij_gra.h5_q, Qij_gra.h95_q, color="purple", alpha=0.2)
ax[1,0].legend()
ax[1,0].grid(alpha=0.2)
...
ax[1,1].plot(np.arange(118)[:85], Qij_gra.mean_q[:85], linestyle="--", color="purple", label="Gravity: SSI="+str(round(SSI_v_gra, 2)))
ax[1,1].fill_between(np.arange(118)[:85], Qij_gra.h5_q[:85], Qij_gra.h95_q[:85], color="purple", alpha=0.2)
ax[1,1].legend()
ax[1,1].grid(alpha=0.2)
...
plt.tight_layout()
plt.savefig("bayesian_models_estimates.png", dpi=300)
plt.show()
plt.close()
