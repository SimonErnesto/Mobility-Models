...
with pm.Model() as mod:
    mu = pm.Uniform("mu", 1e4, 10e4, shape=119)
    fmin = pm.Uniform("fmin", 0.1, 1, shape=119)
    fmax = pm.Uniform("fmax", 1, 100, shape=119)
    v = pm.Deterministic("v", (mu*Ai)/(np.log(fmax/fmin)*rij**2))
    q = pm.Deterministic("q", Ai*(T**-1)*mu/(rj**2))
    alpha1 = pm.HalfNormal("alpha1", 100)
    alpha2 = pm.HalfNormal("alpha2", 10)
    y = pm.NegativeBinomial("y", mu=v, alpha=alpha1, observed=Vij_obs)
    w = pm.NegativeBinomial("w", mu=q, alpha=alpha2, observed=Qij_obs)
...
Vij = pos['v'].values.mean(axis=1)*T

Vij_vis_m = np.sort(Vij)[:118]

Vij_obs = np.sort(data.viajes.values)[:118]

SSI_v_vis = 2*np.sum(np.minimum(Vij_obs, Vij_vis_m))/(np.sum(Vij_obs) + np.sum(Vij_vis_m))

Qij_vis_m =  pos['q'].values.mean(axis=1) #compute number of unique visitors
Qij_vis_m = np.sort(Qij_vis_m)
Qij_obs = np.sort(data.visitors.values)
SSI_q_vis = 2*np.sum(np.minimum(Qij_obs, Qij_vis_m))/(np.sum(Qij_obs) + np.sum(Qij_vis_m))

fig, ax = plt.subplots(nrows=2, ncols=2, figsize=(16,8))
ax[0,0].plot(Vij_obs, Vij_obs, color='k', linestyle=":", label="Observed")
ax[0,0].scatter(Vij_obs, Vij_vis_m, marker="o", facecolor="w", color="g", label="Daily Trips: SSI="+str(round(SSI_v_vis, 2)))
ax[0,0].legend()
ax[0,0].set_title("Average Daily Trips")
ax[0,0].set_ylabel("Estmiate")
ax[0,0].set_xlabel("Observed")
ax[0,1].plot(Vij_obs[:85], Vij_obs[:85], color='k', linestyle=":", label="Observed")
SSI_v_vis = 2*np.sum(np.minimum(Vij_obs[:85], Vij_vis_m[:85]))/(np.sum(Vij_obs[:87]) + np.sum(Vij_vis_m[:85]))
ax[0,1].scatter(Vij_obs[:85], Vij_vis_m[:85], marker="o", facecolor="w", color="g", label="Daily Trips: SSI="+str(round(SSI_v_vis, 2)))
ax[0,1].legend()
ax[0,1].set_title("Average Daily Trips < 50000")
ax[0,1].set_ylabel("Estmiate")
ax[0,1].set_xlabel("Observed")
ax[1,0].plot(Qij_obs, Qij_obs, color='k', linestyle=":", label="Observed")
ax[1,0].scatter(Qij_obs, Qij_vis_m, marker="o", facecolor="w", color="g", label="Daily Visitors: SSI="+str(round(SSI_q_vis, 2)))
ax[1,0].legend()
ax[1,0].set_title("Average Daily Visitors")
ax[1,0].set_ylabel("Estmiate")
ax[1,0].set_xlabel("Observed")
ax[1,1].plot(Qij_obs[:87], Qij_obs[:87], color='k', linestyle=":", label="Observed")
SSI_q_vis = 2*np.sum(np.minimum(Qij_obs[:87], Qij_vis_m[:87]))/(np.sum(Qij_obs[:87]) + np.sum(Qij_vis[:87]))
ax[1,1].scatter(Qij_obs[:87], Qij_vis_m[:87], marker="o", facecolor="w", color="g", label="Daily Visitors: SSI="+str(round(SSI_q_vis, 2)))
ax[1,1].legend()
ax[1,1].set_title("Average Daily Visitors < 15000")
ax[1,1].set_ylabel("Estmiate")
ax[1,1].set_xlabel("Observed")
